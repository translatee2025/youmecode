...
const creatorNav: NavSection[] = [
  ...
  { title: 'Categories & Filters', items: [
        ...
        <ScrollArea className="h-full">
          {nav.map((section) => (
            <SidebarGroup key={section.title}>
              <SidebarGroupLabel className="text-xs uppercase tracking-wider px-4" style={{ color: 'var(--color-text-muted, #888)' }}>
                {section.title}
              </SidebarGroupLabel>
              <SidebarGroupContent>
                <SidebarMenu>
                  {section.items.map((item) => {
                    const isActive = item.url === '/admin' ? currentPath === '/admin' : currentPath.startsWith(item.url);
                    return (
                      <SidebarMenuItem key={item.url}>
                        <SidebarMenuButton asChild>
                          <NavLink to={item.url} end={item.url === '/admin'} className="flex items-center gap-3 px-4 py-2 rounded-lg transition-colors" activeClassName="font-medium"
                            style={isActive ? { background: 'rgba(255,255,255,0.08)', color: 'var(--color-primary, #fff)' } : { color: 'var(--color-text-muted, #888)' }}>
...
function ModeSwitcher({ mode, setMode }: { mode: AdminMode; setMode: (m: AdminMode) => void }) {
  return (
    <div className="flex items-center rounded-lg p-0.5" style={{ background: 'rgba(255,255,255,0.06)' }}>
      <button onClick={() => setMode('creator')} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors', mode === 'creator' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground')}>
        <Paintbrush className="h-3.5 w-3.5" /> Creator
      </button>
      <button onClick={() => setMode('platform')} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors', mode === 'platform' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground')}>
...
function AdminTopBar({ mode, setMode }: { mode: AdminMode; setMode: (m: AdminMode) => void }) {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 border-b shrink-0" style={{ background: 'var(--color-nav, rgba(10,10,10,0.85))', borderColor: 'var(--color-border, rgba(255,255,255,0.1))' }}>
      <div className="flex items-center gap-3">
        <SidebarTrigger />
        <span className="font-semibold text-sm" style={{ color: 'var(--color-text, #f0f0f0)' }}>
          {config.platformName} Admin
        </span>
      </div>
      <div className="flex items-center gap-3">
        <ModeSwitcher mode={mode} setMode={setMode} />
        <div className="h-8 w-8 rounded-full flex items-center justify-center text-xs font-medium" style={{ background: 'rgba(255,255,255,0.1)', color: 'var(--color-text, #f0f0f0)' }}>
...
export default function AdminLayout() {
  const [mode, setMode] = useState<AdminMode>(() => (localStorage.getItem('admin_mode') as AdminMode) || 'creator');
  useEffect(() => { localStorage.setItem('admin_mode', mode); }, [mode]);
