...
const THEME_KEYS = [
  { key: 'color-bg', label: 'Background', default: '#0a0a0a' },
  { key: 'color-card-bg', label: 'Card Background', default: 'rgba(255,255,255,0.06)' },
  { key: 'color-text', label: 'Text', default: '#f0f0f0' },
  { key: 'color-text-muted', label: 'Muted Text', default: '#888888' },
  { key: 'color-primary', label: 'Primary', default: '#ffffff' },
  { key: 'color-nav', label: 'Navigation', default: 'rgba(10,10,10,0.85)' },
  { key: 'color-button', label: 'Button', default: '#ffffff' },
  { key: 'color-border', label: 'Border', default: 'rgba(255,255,255,0.1)' },
];
...
export default function AdminTheme() {
  ...
  useEffect(() => {
    if (!tenant) return;
    supabase.from('theme_settings').select('key, value').eq('tenant_id', tenant.id).then(({ data }) => {
  ...
  const handleSave = async () => {
    ...
    for (const [key, value] of Object.entries(values)) {
      const { data: existing } = await supabase.from('theme_settings').select('id').eq('tenant_id', tenant.id).eq('key', key).maybeSingle();
      if (existing) {
        await supabase.from('theme_settings').update({ value }).eq('id', existing.id);
      } else {
        await supabase.from('theme_settings').insert({ tenant_id: tenant.id, key, value });
      }
    }
    // Apply to document
  ...
  // Check if value looks like a hex color for native color input
  const isHex = (v: string) => /^#[0-9a-fA-F]{6}$/i.test(v);

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold" style={{ color: 'var(--color-text, #f0f0f0)' }}>Theme</h1>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {PRESETS.map(p => (
            <button
              ...
              className="rounded-xl p-3 text-center text-xs font-medium transition-transform hover:scale-105"
              style={{
                background: p.values['color-bg'],
                color: p.values['color-text'],
                border: `2px solid ${p.values['color-border']}`,
              }}
            >
            ...
            <div className="rounded-xl overflow-hidden" style={{ background: values['color-bg'] }}>
              ...
              <div className="p-4">
                <div className="rounded-xl p-4" style={{ background: values['color-card-bg'], border: `1px solid ${values['color-border']}` }}>
                  ...
                  <button className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ background: values['color-button'], color: values['color-bg'] }}>
                    View Details
                  </button>
                </div>
              </div>
